...
| **M14 — Multi-Agent Panel (rule-based orchestrator)** | `PanelNode` + `PanelDecisionNode` — run multiple specialist sub-graphs (parallel or sequential) and merge their verdicts via a user-supplied aggregator. State scoping via `scratch["_panel"]` channel; failure recording at `scratch["_panel_failures"]`; opt-in `required=[...]` and `short_circuit_on=...` escape hatches. `single_llm_specialist()` helper for one-LLM specialist case. **Branch:** `feat/dev/multi-agent-panel` (may ship in v1.1.0 rather than v1.0.0 — v1.0.0 stable is ~90% ready, real-source testing is the remaining gate). Boundary stays clean: Python orchestrator (deterministic), specialists are reusable sub-graphs, no LLM-as-orchestrator. LLM-orchestrator variant (Anthropic-style adaptive dispatch) **deferred to v3.0.0** to preserve the "framework decides, bot executes" narrative. See [plans/2026-06-27-multi-agent-panel-m14.md](plans/2026-06-27-multi-agent-panel-m14.md). | ✅ done | v0.18.0 |
| v2.0.0 bucket | Vector memory, ReflectiveNode-in-YAML, M11 (parallel specialists, paid providers), M12 (Zetryn-hosted models) | 📅 after v1.0.0 stable | — |
