...
| **PL1 — Position Lifecycle Helpers** (position management, NOT entry) | **First position-management agent.** `build_lifecycle()` consumes `PositionContext`, emits `Decision(action ∈ {hold, take_profit, stop_loss, scale_out, exit_full}, size)`. Strictly recommendation — bot owns execution, slippage, MEV, persistence. Boundary-safe (no fetcher, no signer). See [docs/STRATEGIES.md#pl1--position-lifecycle-helpers](STRATEGIES.md#pl1--position-lifecycle-helpers). | 📅 planned | — |
| **S5 — Smart Money Confluence** (entry agent) | **Multi-wallet correlation entry agent** — fires when ≥ N pre-vetted smart wallets buy the same token within a rolling window. Distinct from KOL (multi-wallet correlation vs single-wallet copy). New schemas: `SmartConfluenceEvent`, `SmartWalletRegistry`. See [docs/STRATEGIES.md#s5--smart-money-confluence](STRATEGIES.md#s5--smart-money-confluence). | 📅 planned | — |
