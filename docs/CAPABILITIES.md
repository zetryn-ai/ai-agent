...
| **PL1 — Position Lifecycle Helpers** (position management, NOT a strategy) | **First position-management agent.** `build_lifecycle()` consumes `PositionContext`, emits `Decision(action ∈ {hold, take_profit, stop_loss, scale_out, exit_full}, size)`. Strictly recommendation — bot owns execution, slippage, MEV, persistence. Boundary-safe. NOT in [STRATEGIES.md](STRATEGIES.md) (universal risk-management primitive, not strategy alpha). | 📅 planned | — |
