...
| **PL1 — Position Lifecycle Helpers** | **Recommendation-only TP/SL/exit-scaling nodes** consumed by an entry-agnostic `build_lifecycle()` graph. Inputs: `PositionContext` (bot's open position + live token snapshot). Outputs: `Decision` with `action ∈ {hold, take_profit, stop_loss, scale_out, exit_full}` and `size`. Strictly recommendation — bot owns execution, slippage, MEV. Boundary-safe (no fetcher, no signer). See [docs/STRATEGIES.md](STRATEGIES.md#position-lifecycle-helpers). | 📅 planned | — |
| **Strategy #5 — Smart Money Confluence** | **Multi-wallet smart-money correlation agent** — fires when ≥ N pre-vetted smart wallets buy the same token within a rolling window. Cross-wallet behavioral signal (not tied to a single launch event). New schemas: `SmartConfluenceEvent`, `SmartWalletRegistry`. Reuses sniper safety + market gates. See [docs/STRATEGIES.md](STRATEGIES.md#strategy-5-smart-money-confluence). | 📅 planned | — |
