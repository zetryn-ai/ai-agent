...
| **M14 — Multi-Agent Panel (rule-based orchestrator)** | `PanelNode` + `PanelDecisionNode` — run multiple specialist sub-graphs (parallel or sequential) and merge their verdicts via a user-supplied aggregator. State scoping via `scratch["_panel"]` channel; failure recording at `scratch["_panel_failures"]`; opt-in `required=[...]` and `short_circuit_on=...` escape hatches. `single_llm_specialist()` helper for the one-LLM specialist case. Boundary stays clean: Python orchestrator (deterministic), specialists are reusable sub-graphs, no LLM-as-orchestrator. LLM-orchestrator variant (Anthropic-style adaptive dispatch) **deferred to v3.0.0**. See [plans/2026-06-27-multi-agent-panel-m14.md](plans/2026-06-27-multi-agent-panel-m14.md). | ✅ done | v1.2.0 |
| **Confidence Calibration + Replay Bridge** | `zetryn/analytics/CalibrationMap` — raw LLM score → empirical win rate (histogram bins + shrinkage, per-source fallback, JSON round-trip). Wired via `build_scanner(calibration=)`; fed by the `strategies/backtest.py` bridge: `ClosedTrade`, `fit_calibration`, `replay_dataset`, `closed_trade_metrics` (win-rate/PnL by source + confidence band). See [plans/2026-07-11-confidence-calibration.md](plans/2026-07-11-confidence-calibration.md). | ✅ done | v1.2.0 |
| v2.0.0 bucket | Vector memory, ReflectiveNode-in-YAML, M11 (parallel specialists, paid providers), M12 (Zetryn-hosted models) | 📅 after v1.x | — |
| v3.0.0 bucket | M14.x — LLM-orchestrator variant of panel (Anthropic-style adaptive dispatch). Requires explicit acknowledgment that framework becomes an active reasoner, not just a transparent advisor. | 📅 earned later | — |
