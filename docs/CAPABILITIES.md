...
|---|------------|--------|----------------|-------|
| 1 | Skills / knowledge injection at startup | ✅ | [`zetryn/knowledge/pack.py`](../zetryn/knowledge/pack.py) (`KnowledgePack`) | `KnowledgePack.from_dir(path)` loads markdown → `system_blocks()` and JSON → `lookup(ns, key)`. Tools + memory store still available for non-static knowledge. |
